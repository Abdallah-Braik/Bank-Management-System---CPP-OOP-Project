...
public:
	...
	static clsDate GetStructSystemDate()
	{
		//system date
		time_t t = time(0);
		tm* now = localtime(&t);
	...
	{
		return GetDifferenceInDays(DateOfBirth, clsDate::GetStructSystemDate(), true);
	}
	...
	static string GetSystemDate()
	{
		...
		return to_string(Hour)+":"+to_string(Minute)+"#//#" + to_string(Day) + "/" + to_string(Month) + "/" + to_string(Year);
	}
